import styles from "./SmartToDoList.module.css";
import { useState, type JSX } from "react";
import { useFormik } from "formik";
import * as Yup from "yup";

type Task = {
  id: number;
  text: string;
  completed: boolean;
};

//Объяви функциональный компонент с понятным именем, который
//экспортируется по умолчанию
export default function SmartToDoList(): JSX.Element {
  const [tasks, setTasks] = useState<Task[]>([]);

  // Схема валидации для поля text
  const validationSchema = Yup.object({
    text: Yup.string() //Значение должно быть строкой
      .trim() //Автоматическое обрезание пробелов в начале и конце
      .min(3, "Minimum 3 characters") //Минимальная длина 3 символа с соответствующим сообщением об ошибке
      .max(50, "Maximum 50 characters") //Максимальная длина 50 символов с соответствующим сообщением об ошибке
      .required("Enter the text of the task"), //Обязательное поле с сообщением о необходимости ввода
  });

  // Formik
  const formik = useFormik({
    initialValues: { text: "" }, //Используй хук для работы с формами, передав объект конфигурации: Начальные значения формы: поле "text" как пустая строка

    //Подключи созданную схему валидации
    validationSchema,
    onSubmit: (values, { resetForm }) => {
      const newTask: Task = {
        // уникальный ID на основе времени
        id: Date.now(),
        text: values.text.trim(),
        completed: false,
      };

      setTasks((prev) => [...prev, newTask]);
      resetForm();
    },
  });

  // Переключение статуса выполнения
  // Создай функцию, которая принимает ID задачи,
  // Функция должна обновлять состояние задач:
  // Пройдись по массиву задач
  // Найди задачу с соответствующим ID
  // Инвертируй значение свойства "completed"
  // Верни обновленный массив
  
  const toggleCompleted = (id: number) => {
    setTasks((prev) =>
      prev.map((t) => (t.id === id ? { ...t, completed: !t.completed } : t))
    );
  };

  // Удаление задачи
  // Создай функцию, которая принимает ID задачи для удаления
  // Функция должна обновлять состояние задач:
  // Отфильтруй массив, оставив только задачи, ID которых не совпадает с  переданным
  // Верни новый отфильтрованный массив
  const deleteTask = (id: number) => {
    setTasks((prev) => prev.filter((t) => t.id !== id));
  };

  return (
    //Создай корневой div-контейнер с применением стиля-обертк
    //Добавь заголовок первого уровня с эмодзи списка и названием приложения Примени соответствующий стиль заголовка
    <div className={styles.wrapper}>
      <h1 className={styles.title}>Smart To Do List 📝</h1>
      {/* : Форма ввода новой задачи
Создай форму с обработчиком отправки и примени стиль формы
Внутри формы размести:
Поле ввода текста с:
Типом "text"
Именем "text" (соответствует имени в Formik) */}
      <form onSubmit={formik.handleSubmit} className={styles.form}>
        <input
          type="text"
          name="text"
         
          //Плейсхолдером с приглашением к вводу
          placeholder="Add a new task..."
        
          //Значением из состояния формы
          value={formik.values.text}
        
          //Обработчиком изменения значения
          onChange={formik.handleChange}
         
          //Обработчиком потери фокуса
          onBlur={formik.handleBlur}

          //Стилем для поля ввода
          className={styles.input}
        />

        {/* Кнопку отправки формы с:
Типом "submit"
Текстом "Добавить"
Стилем для кнопки добавления */}
        <button type="submit" className={styles.addButton}>
          Add
        </button>
      </form>
    
     {/* ' : Блок отображения ошибок валидации
Создай условный рендеринг для сообщений об ошибках:
Показывай сообщение только если поле было активировано (touched)
И есть ошибка
Отображай текст ошибки из состояния формы
Примени стиль для сообщений об ошибках' */}
      {formik.touched.text && formik.errors.text ? (
        <div className={styles.error}>{formik.errors.text}</div>
      ) : null}
      
      {/* : Список задач
Создай ненумерованный список с применением стиля списка */}
      <ul className={styles.list}>
        {/* Если массив задач пустой, покажи параграф с сообщением "Список
пуст" Примени соответствующий стиль для пустого состояния
*/}
        {tasks.length === 0 ? (
          <p className={styles.empty}>The list is empty</p>
        ) : (
// Рендеринг отдельных задач
// Для каждой задачи в массиве создай элемент списка с:
// Уникальным ключом на основе ID
// Стилем элемента списка

          tasks.map((task) => (
            <li key={task.id} className={styles.item}>

              {/* : Элемент задачи (текст)
Внутри каждого элемента списка создай span для текста задачи:
Добавь обработчик клика для переключения статуса выполнения
Динамически применяй классы:
Базовый стиль текста задачи
Дополнительный стиль для завершенных задач (условно)
Отображай текст задачи */}

              <span
                role="button"
                tabIndex={0}
                onClick={() => toggleCompleted(task.id)}
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ")
                    toggleCompleted(task.id);
                }}
                className={`${styles.text} ${
                  task.completed ? styles.completed : ""
                }`}
              >
                {task.text}
              </span>

 {/* Кнопка удаления задачи
Рядом с текстом задачи размести кнопку:
Тип кнопки - "button" (чтобы не вызывать отправку формы)
Обработчик клика для удаления задачи
Символ "✕" как содержимое
Стиль для кнопки удаления */}
              <button
                type="button"
                aria-label={`Delete ${task.text}`}
                className={styles.deleteBtn}
                onClick={() => deleteTask(task.id)}
              >
                ✕ DELETE
              </button>
            </li>
          ))
        )}
      </ul>
    </div>
  );
}
